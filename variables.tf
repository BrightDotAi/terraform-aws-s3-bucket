...
variable "cross_account_bucket_policy_stacks" {
  type        = list(string)
  default     = []
  description = <<-EOT
    List of stack references that export cross_account_bucket_policies output.
    Format: tenant/environment/stage/component (e.g. "mgmt/uw2/corp/filestash").
    Policies matching this bucket's account and name are automatically merged into
    the bucket policy via source_policy_documents.
    EOT
}
